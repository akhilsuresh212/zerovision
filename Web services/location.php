...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $latitude = isset($_POST['Latitude']) ? mysqli_real_escape_string($link, $_POST['Latitude']) : "";
    $longitude = isset($_POST['Longitude']) ? mysqli_real_escape_string($link, $_POST['Longitude']) : "";
    ...
    if (isset($_POST['update_location'])) {
        ...
        if ($result) {
            if (mysqli_num_rows($result) > 0) {
                ...
                $sql = " UPDATE `location` SET `lattitude`= '$latitude',`longitude` = '$longitude', ";
                $sql .= " `last_time` = 'CURDATE()' WHERE  `user_id` = '$user_id' ";
                $qur = mysqli_query($link, $sql);
                if ($qur) {
                    $json = array("status" => 1, "msg" => "update location");
                } else {
                    $json = array("status" => 0, "msg" => "Error location update");
                }
            } else {
                $sql = " INSERT INTO `location`(`user_id`,`lattitude`,`longitude`,`last_time` )";
                $sql .= "VALUES('$user_id','$latitude','$longitude','$mob','CURDATE()');";
                $qur = mysqli_query($link, $sql);
                if ($qur) {
                    $json = array("status" => 1, "msg" => "update location");
                } else {
